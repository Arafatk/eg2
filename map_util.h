...
void test() {
    ...
    auto tests_passed = 0;
    auto tests = 11;
    std::cout << "Running 11 simple tests \n";

    auto a = FindWithDefault(m, "foo", "");
    if (a == "") tests_passed++;
    m["foo"] = "bar";
    a =  FindWithDefault(m, "foo", "");
    if (a == "bar") tests_passed++;
    a =  *FindOrNull(m, "foo");
    if (a == "bar") tests_passed++;
    if (m.count("foo") > 0) tests_passed++;
    if (m["foo"] == "bar")  tests_passed++;

    Map m2;
    if ("xyz" == LookupOrInsert(&m2, "foo", "xyz"))tests_passed++;
    if ("xyz" == LookupOrInsert(&m2, "foo", "abc"))tests_passed++;

    typedef std::set<int> Set;
    Set s;
    if(true == InsertIfNotPresent(&s, 0)) tests_passed++;
    if(1 == s.count(0)) tests_passed++;
    if(false == InsertIfNotPresent(&s, 0)) tests_passed++;
    if(1 == s.count(0)) tests_passed++;
    std::cout << tests_passed << " out of " << tests << " tests passed\n" << "\n";
}
}  // namespace gtl
}  // namespace tensorflow
